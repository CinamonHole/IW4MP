...
Start hour: 6:16 PM (Pacific hour)

Purpose: Support non '_load.ff' load screens..
...
void LoadMapLoadscreenHookFunc(XZoneInfo* info, int a2, int a3)
{
	...
	_snprintf(zonePath, sizeof(zonePath) - 1, "%s\\%s\\%s.ff", GetZoneRootHookFunc(zoneDir), zoneDir, info[0].name);

	if (GetFileAttributesA(zonePath) != INVALID_FILE_ATTRIBUTES)
	{
